...
2*(3+4)
  = (term 2) * (expr 3+4)
  = (factor 2) * ((term 3) + (expr 4))
  = (nat 2) * ((factor 3) + (term 4))
  = (nat 2) * ((nat 3) + (factor 4))
  = (nat 2) * ((nat 3) + (nat 4))
  = (nat 2) * (nat 7)
  = nat 14
Then the result nat 14 is parsed as Int type with the return type [(Int, String)]
thus resulting in [(14, "")]
...
1+2*3
  = (term 1) + (expr 2*3)
  = (factor 1) + ((term 2) * (expr 3))
  = (nat 1) + ((factor 2) * (term 3))
  = (nat 1) + ((nat 2) * (factor 3))
  = (nat 1) + ((nat 2) * (nat 3))
  = (nat 1) + (nat 6)
  = nat 7
Then the result nat 7 is parsed as Int type with the return type [(Int, String)]
thus resulting in [(14, "")]
-}
...
val:: Parser Expr
val = do{x <- some digit; return (Val(read x))}
